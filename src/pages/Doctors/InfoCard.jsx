...
const InfoCard = ({ doctorDetails }) => {
  ...
  return (
    <Box className={classes.container}>
      <Box display="flex">
        <Avatar alt="Remy Sharp" src={doctorDetails.photo} className={classes.large} />

        <Box style={{ marginLeft: 13, marginTop: 7 }}>
          <Typography className={classes.name}>{doctorDetails.doctorName}</Typography>
          <Typography className={classes.desgination}>{doctorDetails.speciality}</Typography>
        </Box>
      </Box>
      ...
      <Box marginTop={0.5} display="flex" justifyContent="flex-end">
        <Button className={classes.button} onClick={handeOnClick} variant="outlined">
          Settings
        </Button>
      </Box>
    </Box>
